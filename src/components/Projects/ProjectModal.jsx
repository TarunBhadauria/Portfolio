...
const ProjectModal = ({ data, setShow, show }) => {
  ...
  const handlePrevImage = () => {
    const currentIndex = data.moreImages.indexOf(image);
    const prevIndex =
      (currentIndex - 1 + data.moreImages.length) % data.moreImages.length;
  ...
  return (
    <div
      className={`${style.ProjectModal} ${
        show ? style.showModal : style.hideModal
      }`}
      ref={modalRef}
    >
        ...
        <div className={style.thumbnailImage}>
          ...
          <div className={style.thumbnail} ref={thumbnailRef}>
            {data.moreImages.map((i, index) => (
              <img
                onLoad={() => setAllLoad(!allLoad)}
                className={image === i ? style.selected : ""}
                key={index}
                src={i}
      ...
      <div className={style.information}>
        ...
        <div>
          ...
          <div className={style.links}>
            <a href={data.githubLink} className={style.links } target="_blank" rel=" noreferrer">
              <FaGithub />
            </a>

            <FaYoutube />

            <a href={data.liveLink} className={style.links} target="_blank" rel=" noreferrer">
              <FaLink />
            </a>
          </div>
        </div>
